...
# Read-only compatibility inventory for the owner-accepted CentOS 7 student lab.
# This script does not install packages, load modules, change sysctls or stop services.
...
run_readonly cat /etc/centos-release
...
run_readonly lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT
...
if [[ "${kernel_major}" =~ ^[0-9]+$ && "${kernel_minor}" =~ ^[0-9]+$ ]] &&
  ...
  echo "calico_current_kernel_gate=PASS (${kernel_numeric} >= 5.10)"
else
  echo "calico_current_kernel_gate=REVIEW (${kernel_numeric} is below the generic 5.10 baseline)"
fi
...
run_readonly rpm -q chrony iptables ipset conntrack-tools socat ebtables ethtool containerd.io kubelet kubeadm kubectl
...
echo "This report is diagnostic evidence only. It does not approve installation or bypass kubeadm/CNI preflight failures."
