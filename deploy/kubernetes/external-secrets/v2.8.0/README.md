...
本目录记录学生集群使用的 External Secrets Operator 版本与安装边界，不保存上游发布清单、镜像层或任何凭据。
...
3. 下载并校验官方发布清单；
4. 把上游清单中的 `default` Namespace 字段、Namespace 参数及 Webhook DNS 名称精确归一化为 `external-secrets`，任何预期数量或结果不一致都会让工作流失败；
...
安装脚本会再次审计 Webhook DNS 名称，拒绝仍使用 `external-secrets-webhook.default.svc` 的错误安装包。正确名称必须是 `external-secrets-webhook.external-secrets.svc`。
